...
import expensive.main.baritone.utils.accessor.IPlayerControllerMP;
import expensive.modules.api.FunctionRegistry;
import expensive.modules.impl.combat.KillAura;
import expensive.modules.impl.misc.ClickSettings;
import expensive.modules.impl.player.NoInteract;
...
public class PlayerController implements IPlayerControllerMP {
    ...
    public void attackEntity(PlayerEntity playerIn, Entity targetEntity) {
        ...
        FunctionRegistry functionregistry = Expensive.getInstance().getFunctionRegistry();
        ClickSettings clicksettings = functionregistry.getClickSettings();
        this.syncCurrentPlayItem();

        if (clicksettings.mode.is("Пакетный")) {
        ...
        if (clicksettings.mode.is("Легитный")) {
            this.connection.sendPacket(new CUseEntityPacket(targetEntity, playerIn.isSneaking()));
